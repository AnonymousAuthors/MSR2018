...
handler = RotatingFileHandler('/xxx/logfile', maxBytes=500000, backupCount=1)
...
def explain_arg_pack(args):
    from ir_summary_pre import IRSummaryPre 
    tags = IRSummaryPre.checkTag(args[SUMMARY])
    dirtyWords = IRSummaryPre.checkDirtyWords(args[SUMMARY])
    return jsonify(keyword=args[KEYWORD], sentences=args[SENTENCES], duplicates=args[DUPLICATES],\
            tip=give_tip(args[DUPLICATES].__len__(), tags,dirtyWords,args[SENTENCES].__len__()), \
...
def give_tip(dup_length,tags,dirtyWords, sim_length = 5):
    tips = ''
    if len(tags) > 0:
        tips = tips + "Tag --"
        for tag in tags:
            tips = tips + ' ' + tag
        tips = tips + " may be tags. Do you need the formation of \"[tag]\". \n"
    if len(dirtyWords) > 0:
        tips = tips + "dirtyWords --"
        for words in dirtyWords:
            tips = tips + ' ' + words
        tips = tips + " exist(s). Please replace these words. \n"
    if dup_length > 0:
        return tips + 'Some existing reports are quite alike.'
    elif sim_length > 0:
        return tips + 'Could you give us more information?'
    elif len(tips) > 0:
        return tips
    else:
...
@app.route('/ir_demo/feedback', methods=['GET', 'POST'])
def feedback():
    """
    Create a new report.
    ...
    # recommend
    tags = IRSummaryPre.checkTag(summary)
    dirtyWords = IRSummaryPre.checkDirtyWords(summary)
